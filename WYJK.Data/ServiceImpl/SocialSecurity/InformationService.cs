...
namespace WYJK.Data.ServiceImpl
{
    ...
    public class InformationService : IInformationService
    {
        ...
        {
            string sql = $"select ID,Name,ImgUrl,Type,CreateTime,Row from (select ROW_NUMBER() OVER(ORDER BY i.ID desc )AS Row, i.* from Information i where Name like @Name and Type = @Type) ii where ii.Row between @StartIndex AND @EndIndex order by ii.ID desc";
        ...
        {
            string sql = $"insert into Information(Name,Type,ImgUrl,StrContent,CreateTime) values('{model.Name}','{model.Type}','{model.ImgUrl}','{model.StrContent}','{model.CreateTime}')";
