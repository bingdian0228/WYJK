...
namespace WYJK.Data.ServiceImpl
{
    ...
    public class InformationService : IInformationService
    {
        ...
        {
            string sql = $"select * from (select ROW_NUMBER() OVER(ORDER BY i.ID )AS Row,i.* from Information i where Name like @Name and Type={(int)InfoTypeEnum.InsuredIntroduce}) ii where ii.Row between @StartIndex AND @EndIndex";
            ...
            int totalCount = await DbHelper.QuerySingleAsync<int>($"select count(0) from Information where Name like @Name  and Type={parameter.Type} ", new { Name = "%" + parameter.Name + "%" });
        ...
        public async Task<bool> InformationAdd(Information model)
        {
            ...
            string sql = $"insert into Information(Name,ImgUrl,StrContent,Type) values('{model.Name}','{ImgUrls}','{model.StrContent}',{model.Type})";
